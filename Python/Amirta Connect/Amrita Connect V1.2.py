...
def changebio():
    ...
    chna=Button(edbitk,text="Edit User-Name",bg="Coral1",fg="White",command=changeunnametk)
    chna.place(x=450,y=80)
...
def myprofile(usernam):
    ...
    chpr=Button(mypr,text="Edit Bio",bg="Coral1",fg="White",command=closemyprtochangebio)
    chpr.place(x=200,y=570)
    back=Button(mypr,text="Back",bg="Coral1",font=20,fg="White",width=15,bd=1,height=1,command=closemyprofile)
    back.place(x=20,y=620)
    mypr.mainloop()
    amritaconnect()
def submitmessage():
    global mess,myfrndsch
    print(mess.get())
def chatwithfriend():
    global myid,requestid,mess,urpr
    urpr.destroy()
    if myid<requestid:
    ...
    Logo.place(x=0,y=20)
    back=Button(myfrndsch,text="Back",bg="Coral1",font=13,fg="White",width=15,bd=1,height=1,command=backtoamc)
    back.place(x=50,y=400)
    cursor.execute("select * from {}".format(usetablen))
    msgs=cursor.fetchall()
    scrollbar =Scrollbar(myfrndstk,width=25)
    scrollbar.pack(side = RIGHT, fill=Y )
    mychat= Listbox(myfrndsch,yscrollcommand = scrollbar.set ,font=20,bg="Coral1",fg="White",width=20)
    if len(msgs)!=0:
        msgs=eval(msgs[0])
        for message in msgs:
    ...
    messl.place(x=20,y=150)
    meess=Entry(myfrndsch,bd=2,width=30)
    mess.place(x=150,y=90)
    messageb=Button(myfrndsch,text="Enter",bg="Coral1",font=13,fg="White",width=15,bd=1,height=1,command=submitmessage)
    messageb.place(x=50,y=340)
    messagel.place(x=100,y=150)
    myfrndstk.mainloop()
...
def viewpro():
    ...
    chat=Button(urpr,text="Chat",command=chatwithfriend)
    if openw==1:
        chat.place(x=400,y=500)
    else:
...
def friendreq():
    ...
    declineb=Button(popre,text="Decline",command=declinerq)
...
def myfriendswin():
    ...
    searchb.place(x=50,y=340)
    searchn.place(x=100,y=150)
    myfrndstk.mainloop()
def amritatk():
    ...
    myfrnds=Button(amc,text="My Friends",command=myfriendscloseamc)
    myfrnds.place(x=400,y=400)
    amritaconnect()
    amc.mainloop()
def amritaconnect():
...
def photoupload():
    ...
    ph=askopenfilename()
def nextco():
    global main,signupwin,namee,unamee,gen,agee,rpas,pase,ph,username,bioe,seme,coure
                        ...
                        if gen.get()!=0:
                            ...
                            if 0<len(bioe.get())<=150:
                                if 0<len(coure.get())<=20:
                                    if 0<len(seme.get())<=10:
                                        if len(ph)!=0:
                                            l=[namee.get(),unamee.get(),agee.get(),pase.get(),gen.get()]
                                            print("done")
                                            cursor.execute("use amrita_connect")
                                            cursor.execute("insert into users_list(username,name,password,image,intrest,count,gender,bio,age,semester,course) values('{}','{}','{}','{}','hghg',0,'{}','{}',{},'{}','{}')".format(unamee.get(),namee.get(),pase.get(),ph,gen.get(),bioe.get(),agee.get(),seme.get(),coure.get()))
                                            sql.commit()
                                            username=unamee.get()
                                            signupwin.destroy()
        #                                   myprofile(username)
                                        ...
                                        if len(seme.get())==0:
                                            ...
                                            pop("Please Enter Your Sem Name In Less Than 20 Characters")
                                else:
                                    if len(coure.get())==0:
...
def signuppg1():
    global main,signupwin,namee,unamee,gen,agee,pase,rpase,bioe,seme,coure
    main.destroy()
    signupwin=Tk()
    gen=IntVar()
